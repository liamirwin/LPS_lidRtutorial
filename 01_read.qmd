...
Often we do not need the entire set of attributes loaded into memory, or may want a particular subset of points. We can use `Classification` as well as other attributes to filter points.
...
Here we load a subset of the lidar points based on certain criteria using the `filter` argument in directly in `readLAS()`.

In this case we use a pre-existing filter during to subset only first returns-to see all run `readLAS(filter = "-help")` for a full list of these filters.
...
Visualize this filtered dataset using the `plot()` function.
...
# Load another version of the point cloud and visualize with an applied filter to isolate ground points
