...
#| fig-cap: "Raster of maximum point height (Z) calculated from the `LAScatalog` at a 20m resolution."
...
#| fig-cap: "Raster of maximum point height (Z) using only first returns, calculated from the `LAScatalog` at 20m resolution."
...
Run `parallel::detectCores()` to see avaliable workers
:::
...
We define how the LAScatalog should be broken down into chunks for processing. A 10m buffer is added to each chunk to avoid edge artifacts when a function's calculations depend on neighboring points (also automatically applied by default).

```{r specify_catalog_options_parallel}
#| label: fig-catalog-chunks
#| fig-cap: "Visualization of the `LAScatalog` processing chunks, with a size of 500m and a 10m buffer."
# Specify options
opt_select(ctg) <- "xyz"
...
end_time <- Sys.time()
cat("Single core processing took:", round(end_time - start_time, 2), "seconds\n")
...
end_time <- Sys.time()
cat("Parallel processing took:", round(end_time - start_time, 2), "seconds\n")
