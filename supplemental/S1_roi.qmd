...
Now, let's read a shapefile called `roi.gpkg`` using `sf::st_read()` and plot it on top of the LiDAR header plot.

``` r
# Read the shapefile "roi.gpkg" using st_read()
plots <- sf::st_read(dsn = "data/roi/roi.gpkg", quiet = TRUE)
...
Clip a transect from A `c(2670525, 1258740)` to B `c(2670740, 1258970)`.

#### E3.

Clip a transect from A `c(2670525, 1258740)` to B `c(2670740, 1258970)` but reorient it so it is no longer on the XY diagonal. Hint = `?clip_transect`
